...
describe('the write gate', () => {
  ...
  it('refuses without the image that was read, so a blind full write is impossible', async () => {
    // Sending every block would rewrite six kilobytes including parts of memory
    // nobody has modelled. The base is what keeps a write to what changed.
    const img = image()
    const ident = {
    ...
    ).rejects.toThrow(/needs the image that was read/)
  })
})
