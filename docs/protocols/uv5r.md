...
the GT-5R/UV-5RX3/UV-5G Pro question are exactly where they were.
...
- **No reader outside the app has taken these bytes off the wire.** The two
  ...
  opinion on the *decode* and not on the read - a cabled read by CHIRP itself
  is still the missing step.
...
3. **Two reads, compared** - done, byte-identical. The capture is committed as
   `test/fixtures/images/uv5r-HN5RV011.bin` with the cross-check fixtures
   regenerated by `scripts/dump-uv5r-channels.py` and `dump-uv5r-settings.py`,
   and `test/lib/radios/uv5r/hardware-fixture.spec.ts` holds boofwang's decode
   against CHIRP's over those bytes: 21 channels and 50 settings fields, field
   for field. It was scanned first, which is what any capture needs before it
   is committed - every channel is unnamed, the power-on message is the factory
   `WELCOME`, and the DTMF and ANI code region is byte-identical to the UV-5G
   capture already in the tree, so those are defaults rather than somebody's
   identity. Still outstanding: a read by an independent reader outside the
   app, matching sha256.
